...
import random 
import numpy as np
#%% Section 1: we define a function that represents a game. 
# The function only has one variable Edge. 
# Edge variable is set up in a way that to increase the chance of winning, we need to decrease Edge value.
def Game(Edge):
...
# one with (1) no strategy (2) Martingale strategy (3) PP Strategy
...
def PP_Strategy(Nn,dNn, fund, initial_gambler, gambler_count, Edge, C,alpha, beta, gamma, delta, dt, x0, y0):
 ...
 # Number of iterations for the PP model is picked randomly using random function.
